...
import {
  DEFAULT_CONFIG,
  type Config,
} from "@/lib/bots/crypto-shares/forecast/config";
import {
  HORIZONS,
  FRAMES,
  type Horizon,
} from "@/lib/bots/crypto-shares/forecast/identity";
import type { State, Closed } from "@/lib/bots/crypto-shares/forecast/state";
import type { Candle } from "@/lib/bots/crypto-shares/forecast/candles";
import {
  nextEntryAt,
  nextStake,
  type Signal,
  type Zone,
} from "@/lib/bots/crypto-shares/forecast/rules";
type Run = {
  id: string;
  mode: "paper" | "live";
  revision: number;
...
export function ForecastBot() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      pending = true;
      try {
        const result = await requestJSON<Response>(`/api/forecast?page=${page}`);
        if (alive) {
          setData(result);
          setError("");
        }
  ...
  async function submit(payload: Record<string, unknown>) {
    setBusy(true);
    try {
      await requestJSON("/api/forecast", payload);
      setData(await requestJSON<Response>(`/api/forecast?page=${page}`));
      setEditing(undefined);
      setReview(null);
      setKey("");
      toast.success(
        payload.action === "deploy"
          ? "Forecast deployed with ARM off"
  ...
  return (
    <section className="panel scalper-bot forecast-bot">
      <div className="row-between forecast-heading">
        <div className="continuation-brand">
          ...
          <div>
            ...
            <h2>
              Forecast <span className="tag">BTC · 5m / 15m / 1h</span>
            </h2>
          </div>
        </div>
        <button
      ...
      <p className="scalper-description">
        Forecast the next BTC candle with 1m, 5m, 15m and 1h trend,
        support/resistance and candle patterns. Orders enter before T−20.
      </p>
      ...
      <details className="cs-details">
        <summary>Research check · no proven winning edge</summary>
        <p className="small muted">August 1–September 11, 2026 BTC spot proxy: 5m 48.4% (748 signals), 15m 42.3% (293), 1h 46.9% (64). This did not replay Polymarket settlement, quotes or fills. It is not a trading return or an expected live win rate.</p>
      </details>
      {error && (
              ...
              <div className="scalper-chart-top">
                <div>
                  ...
                  <p className="small muted">
                    {h === 3600
                      ? "Binance BTC/USDT candles · hourly settlement"
                      : "Binance BTC/USDT signal · Chainlink 60-second TWAP settlement"}
                  </p>
                </div>
                <span className="tag">
              ...
              <div className="scalper-signal">
                ...
                <span>
                  {online
                    ...
                    : data?.feed?.message ||
                      "The engine loads verified closed BTC candles before trading."}
                </span>
              </div>
        ...
        return (
          <div className="scalper-run" key={run.id}>
            <div className="row-between">
              ...
              <div className="cs-controls">
                <button
                  ...
                  aria-label={`${run.mode} Forecast settings`}
            ...
            <fieldset className="scalper-horizons">
              ...
              {HORIZONS.map(h => <label key={h}>
                <input type="checkbox" role="switch" aria-label={`${run.mode} Forecast ${label(h)} entries`}
                  checked={s.config.horizons.includes(h)} disabled={busy}
                  onChange={e => void submit({action:"window",runId:run.id,revision:run.revision,horizon:h,enabled:e.target.checked})} />
                {label(h)} {s.config.horizons.includes(h) ? "On" : "Off"}
              </label>)}
            </fieldset>
            <p className="small muted">Window switches control new entries. Open positions continue to settle. ARM remains {s.armed ? "on" : "off"}.</p>
            <div className="continuation-metrics">
              <div>
                <span>
                  {run.mode === "paper"
            ...
            <div className="scalper-lots">
              {s.config.horizons.map((h) => (
                <span key={h}>
                  {label(h)} next lot{" "}
                  <strong>
                    {nextStake(s.config) === null
                      ? "Limit reached"
                      : money(nextStake(s.config)!)}
                  </strong>
                </span>
              ))}
              <span>Fixed stake · no loss doubling</span>
            </div>
      ...
      >
        <DialogContent className="app-dialog scalper-settings">
          <DialogHeader>
            ...
            <DialogDescription>
              Four-timeframe BTC forecasts. Orders target the upcoming round
          ...
          >
            ...
            {mode === "live" && (
              <>
                ...
                <p className="small muted">
                  ...
                  The BTC spot chart is a signal source. Live ARM requires a
            ...
            <p className="small muted">
              ...
              risk settings can change. Window switches can change immediately.
            </p>
            <button
              className="button"
              disabled={busy}
      ...
      >
        <DialogContent className="app-dialog">
          <DialogHeader>
            <DialogTitle>Arm Forecast live</DialogTitle>
            <DialogDescription>
          ...
          {review && (
            <>
              <p>
                ...
                {money(review.state.config.baseLotCents)} · Fixed stake
              </p>
              <p className="small muted">
  ...
  if (complete.length < 2)
    return (
      <div className="scalper-chart-empty">
        ...
        <p>
          Analysis uses at least 55 closed candles per timeframe. Gaps are never filled
  ...
  return (
    <div className="scalper-chart">
      <svg
        ...
        role="img"
        aria-label="Closed Binance BTC candles with confirmed support and resistance zones"
      >
        {[0, 0.5, 1].map((n) => {
