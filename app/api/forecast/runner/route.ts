...
import {
  forecastBot,
  forecastFeed,
  requireForecastRunner,
  mutateForecast,
  type RunRow,
} from "@/lib/server/forecast";
import { FORECAST } from "@/lib/bots/crypto-shares/forecast/identity";
import {
  commandSchema,
  type State,
} from "@/lib/bots/crypto-shares/forecast/state";
import { analyze, quoteBuy } from "@/lib/bots/crypto-shares/forecast/rules";
...
export async function GET(req: Request) {
  try {
    await requireForecastRunner(req);
    const b = await forecastBot(),
      db = database(),
      p = await settings();
    const [rows, feed] = await Promise.all([
      db
        .prepare(
          "SELECT r.*,p.status member_status FROM forecast_runs r JOIN profiles p ON p.user_id=r.user_id WHERE r.bot_id=?",
        )
        .bind(b.id)
...
export async function POST(req: Request) {
  try {
    ...
    if (c.action === "heartbeat") {
      ...
      await db
        .prepare(
          "INSERT OR IGNORE INTO forecast_feed(id,lease,lease_until,heartbeat,data_json) VALUES(?,?,0,0,?)",
        )
        .bind(FORECAST.key, c.lease, "{}")
        .run();
      const result = await db
        .prepare(
          "UPDATE forecast_feed SET lease=?,lease_until=?,heartbeat=?,data_json=? WHERE id=? AND (lease=? OR lease_until<?)",
        )
        .bind(
          c.lease,
          now + 15000,
      ...
      if (!result.meta.changes)
        throw new HttpError(409, "Another Forecast runner owns the lease");
      return json({ ok: true });
    }
    if (
      !(await db
        .prepare(
          "SELECT id FROM forecast_feed WHERE id=? AND lease=? AND lease_until>?",
        )
        .bind(FORECAST.key, c.lease, now)
        .first())
    )
      throw new HttpError(409, "Runner lease expired");
    if (["arm", "disarm", "configure", "window"].includes(c.command.action))
      throw new HttpError(403, "Only the member can change ARM and settings");
    const b = await forecastBot(),
      r = await db
        .prepare("SELECT * FROM forecast_runs WHERE id=? AND bot_id=?")
        .bind(c.runId, b.id)
        .first<RunRow>();
    if (!r) throw new HttpError(404, "Forecast deployment not found");
    if (
      await db
    ...
    return json(
      await mutateForecast(r, c.eventId, c.command, {
        liveAllowed,
        signal,
