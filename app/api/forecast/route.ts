...
import {
  initialState,
  type State,
} from "@/lib/bots/crypto-shares/forecast/state";
import { HORIZONS } from "@/lib/bots/crypto-shares/forecast/identity";
import { analyze } from "@/lib/bots/crypto-shares/forecast/rules";
const wallet = {
  walletAddress: z
    .string()
...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({action: z.literal("window"), runId:z.string().uuid(),revision:z.number().int(),horizon:z.union([z.literal(300),z.literal(900),z.literal(3600)]),enabled:z.boolean()}).strict(),
]);
export async function GET(req: Request) {
  try {
    ...
    const [rows, feed] = await Promise.all([
      db
        .prepare(
          "SELECT * FROM forecast_runs WHERE user_id=? AND bot_id=? ORDER BY created_at",
        )
        .bind(u.id, b.id)
        .all<RunRow>(),
      forecastFeed(),
    ...
    const runs = await Promise.all(
      rows.results.map(async (r) => {
        const history = await db
          .prepare(
            "SELECT id,kind,data_json,created_at FROM forecast_events WHERE run_id=? AND kind IN ('closed','fill','unfilled','uncertain') ORDER BY revision DESC LIMIT 30 OFFSET ?",
          )
          .bind(r.id, (page - 1) * 30)
          .all<{
            id: string;
...
export async function POST(req: Request) {
  try {
    ...
    if (
      c.action !== "disarm" && !(c.action === "window" && !c.enabled) &&
      (!platform.deploymentsOpen ||
        platform.maintenanceMode ||
        b.status !== "published")
    )
      throw new HttpError(403, "Forecast deployments are paused");
    ...
    if ("walletKey" in c || "walletAddress" in c) {
      ...
      if (c.walletKey && c.walletAddress)
        credentials = {
          ...
          cipher: await sealForecastWallet(c.walletKey, u.id, b.id),
        };
    }
    if (c.action === "deploy") {
      ...
      const existing = await db
        .prepare(
          "SELECT id FROM forecast_runs WHERE user_id=? AND bot_id=? AND mode=?",
        )
      ...
      await db.batch([
        db
          .prepare(
            "INSERT INTO forecast_runs(id,user_id,bot_id,mode,revision,last_event,state_json,wallet_address,wallet_cipher,created_at,updated_at) VALUES(?,?,?,?,0,?,?,?,?,?,?)",
          )
          .bind(
            id,
            u.id,
        ...
        auditStatement(u.id, "forecast.deployed", id, { mode: c.mode }),
      ]);
      return json({ ok: true, id }, 201);
    }
    let row = await db
      .prepare(
        "SELECT * FROM forecast_runs WHERE id=? AND user_id=? AND bot_id=?",
      )
      .bind(c.runId, u.id, b.id)
      .first<RunRow>();
    if (!row) throw new HttpError(404, "Forecast deployment not found");
    ...
    if (c.action === "disarm") {
      for (let i = 0; i < 4; i++)
        try {
          return json(
            await mutateForecast(row, crypto.randomUUID(), { action: "disarm" }),
          );
        } catch (e) {
    ...
    const feed = await forecastFeed();
    return json(
      await mutateForecast(
        row,
        crypto.randomUUID(),
        c.action === "window" ? {action:"window",horizon:c.horizon,enabled:c.enabled} : c.action === "configure"
