  ...
  try {
    ...
    if (input.action === "save-bot") {
      if (input.id) {
        ...
        if (scoped?.strategy_key === "crypto-shares.forecast.confluence" && (input.name !== "Forecast" || input.family !== "Crypto Shares"))
          throw new HttpError(409, "Forecast must retain its exact Crypto Shares catalog identity");
