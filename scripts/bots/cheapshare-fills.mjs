...
export function signedOrderId(signed, exchange) {
  ...
  return hashTypedData({
    ...
    types: { Order: fields },
    primaryType: "Order",
    message: { ...signed, side: signed.side === "BUY" ? 0 : 1 },
  });
}
...
const safe = (value) => {
  ...
  if (value < 0n || !Number.isSafeInteger(result))
    throw new Error("Unsupported fill amount");
  return result;
};
export function receiptFills({
  ...
  for (const log of receipt.logs) {
    ...
    const a = event.args;
    if (
      a.orderHash.toLowerCase() !== orderId.toLowerCase() ||
      a.maker.toLowerCase() !== wallet.toLowerCase() ||
      a.tokenId !== BigInt(tokenId) ||
      Number(a.side) !== (side === "BUY" ? 0 : 1)
    )
      continue;
    ...
    fills.push({
      id: `${receipt.transactionHash.toLowerCase()}:${log.logIndex}`,
      orderId,
      side,
      tokenId,
      transactionHash: receipt.transactionHash,
      logIndex: log.logIndex,
      blockNumber: safe(receipt.blockNumber),
      grossMicros: safe(gross),
      sharesMicros: safe(shares),
      feeMicros: safe(a.fee),
      cashMicros: safe(cash),
...
export function summarizeFills(fills, side = "BUY") {
  ...
  for (const fill of fills) {
    ...
    if (old && JSON.stringify(old) !== JSON.stringify(fill))
      throw new Error("Conflicting execution event");
    unique.set(fill.id, fill);
  }
  const sum = (key) =>
    safe([...unique.values()].reduce((n, f) => n + BigInt(f[key]), 0n));
  return {
    costMicros: sum("cashMicros"),
    sharesMicros: sum("sharesMicros"),
    feeMicros: sum("feeMicros"),
    fills: [...unique.values()],
  };
}
export function realizedSalePnl(
  entryCostMicros,
  ownedSharesMicros,
  soldSharesMicros,
  actualNetProceedsMicros,
) {
  if (
    soldSharesMicros > ownedSharesMicros ||
    soldSharesMicros <= 0 ||
    ownedSharesMicros <= 0
  )
    throw new Error("Sale exceeds this bot's shares");
  const allocatedCost =
    (BigInt(entryCostMicros) * BigInt(soldSharesMicros)) /
    BigInt(ownedSharesMicros);
  return {
    allocatedCostMicros: safe(allocatedCost),
    pnlMicros: actualNetProceedsMicros - safe(allocatedCost),
  };
}
