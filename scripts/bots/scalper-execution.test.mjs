...
import {
  ...
  summarizeFills,
  signedOrderId,
} from "./scalper-fills.mjs";
import { submitBuy } from "./scalper-live.mjs";
...
test("Scalper attributes exact confirmed receipts and deduplicates fills across replays", () => {
  const fills = parse([
    log(other, 0),
    log(own, 1),
...
test("Scalper persists a signature-independent hash bound to order amounts and exchange", () => {
  const signed = {
    salt: 1n,
    maker: wallet,
    signer: wallet,
...
test("Scalper never posts after its deadline and does not retry an uncertain submission", async () => {
  let calls = 0;
  const prepared = {
    signed: {},
    client: {
