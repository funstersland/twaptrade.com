...
export function entryGate(
  ...
  check(
    !s.positions.some((p) => p.status === "uncertain"),
    "Resolve the uncertain order before any new entry",
  );
  ...
  check(
    !s.positions.some((p) => p.market.horizon === market.horizon),
    "Resolve the previous position in this timeframe first",
  );
  const stake = nextStake(s.config, s.lossStreaks[market.horizon]);
  check(stake !== null, "Martingale step or maximum stake limit reached");
...
export function reduce(
  ...
  if (c.action === "configure") {
    ...
    s.message = "Waiting for a qualified rejection at T-20";
    ...
    if (c.action === "submit") {
      check(s.armed && p.status === "prepared", "Order cannot be submitted");
      check(
