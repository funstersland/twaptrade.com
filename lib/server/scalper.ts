...
import { SCALPER } from "../bots/crypto-shares/scalper/identity.ts";
import { reduce, type Command } from "../bots/crypto-shares/scalper/state.ts";
import type { Candle, Tick } from "../bots/crypto-shares/scalper/candles.ts";
import type { Signal } from "../bots/crypto-shares/scalper/rules.ts";
export type RunRow = {
  id: string;
  user_id: string;
  bot_id: string;
...
export async function scalperBot() {
  const rows = await database()
    ...
    .bind(SCALPER.family, SCALPER.name, SCALPER.key)
    .all<{ id: string; status: string }>();
  if (rows.results.length !== 1)
    throw new HttpError(
      404,
      "Scalper is not registered or its catalog identity needs review",
    );
  return rows.results[0];
}
export async function sealScalperWallet(
  keyText: string,
  userId: string,
  botId: string,
) {
...
export async function requireScalperRunner(req: Request) {
  const secret = env.TWAP_SCALPER_RUNNER_TOKEN;
...
export async function scalperFeed() {
  const row = await database()
    .prepare("SELECT heartbeat,data_json FROM scalper_feed WHERE id=?")
    .bind(SCALPER.key)
...
export async function mutateScalper(
  row: RunRow,
  eventId: string,
  command: Command,
  ...
  const update = wallet
    ? "UPDATE scalper_runs SET state_json=?,revision=?,last_event=?,updated_at=?,wallet_address=?,wallet_cipher=? WHERE id=? AND revision=?"
    : "UPDATE scalper_runs SET state_json=?,revision=?,last_event=?,updated_at=? WHERE id=? AND revision=?";
  const rows = await db.batch([
    ...
    db
      .prepare(
        "INSERT OR IGNORE INTO scalper_events(id,run_id,revision,kind,data_json,created_at) SELECT ?,id,revision,?,?,? FROM scalper_runs WHERE id=? AND last_event=?",
      )
      .bind(
        eventId,
