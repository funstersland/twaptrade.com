...
Use `prices.crypto.chainlink.twap`, `windowSeconds:60`, `btc/usd`, with exact E18 arithmetic. These are candles of observed TWAP values, not a recreation of Chainlink's underlying TWAP formula. Build closed 15s, 1m, 3m and 5m candles from the stream. Reject future/out-of-order observations and arrivals more than 60 seconds late. Bounded delayed observations may complete historical candles, but entry still requires a live observation no older than three seconds. More than three seconds between observation timestamps invalidates a candle. Finalization requires the next observation across its closing boundary. Persist completed candles in `scalper_feed`; partial candles are not restored after a restart. Missing history is never backfilled with spot prices or fabricated observations.
